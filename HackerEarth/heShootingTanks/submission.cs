...
partial class Solution
{
    ...
    public class MatrixSum
    {
        ...
        public MatrixSum(int x, int y)
        {
            _matrix = new T[x + 1, y + 1];
        }
        ...
        public MatrixSum(int[,] matrix)
        {
            ...
            var m = _matrix = new T[rows + 1, cols + 1];
        ...
        {
            var m = _matrix;
            int rows = m.GetLength(0) - 1;
            int cols = m.GetLength(1) - 1;
        ...
        public T GetSum(int x, int y, int dx, int dy)
        {
            return this[x, y, x + dx - 1, y + dy - 1];
        }

        public T GetSumInclusive(int x, int y, int x2, int y2)
        {
            return this[x, y, x2, y2];
        }
        ...
        public void SetRange(int x1, int y1, int x2, int y2, T value)
        {
            for (int i = x1; i <= x2; i++)
                for (int j = y1; j <= y2; j++)
                    this[i, j] = value;
