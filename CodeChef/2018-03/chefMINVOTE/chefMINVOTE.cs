...
partial class Solution
{
    ...
    public void Solve()
    {
        ...
        while (t-- > 0)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                int left = Bound(pre, pre[i] - s[i], 0, i) - 1;
                if (left < 0) left = 0;
                if (left <= i - 1)
                {
                    table[left]++;
                ...
                int right = Bound(pre, pre[i+1] + s[i], i, n-1, true)-1;
                if (i + 1 <= right)
                {
                    table[i + 1]++;
                    table[right + 1]--;
            ...
            WriteLine(string.Join(" ", table.Take(n)));
        }
    }
    ...
    static int Bound<T>(T[] array, T value, int left, int right, bool upper = false)
