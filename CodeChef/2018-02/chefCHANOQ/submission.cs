...
partial class Solution
{
    ...
    void TestData()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            var low = random.Next(1, 999);
            var hi = random.Next(1, 999);
            if (hi<low) Swap(ref hi, ref low);
            segs.Add(new Segment { L = low, R = hi, W = 1, I = i });
        }
        ...
        for (int i = 0; i < q; i++)
        {
            hashset.Clear();
            for (int j = 0; j < 50; j++)
                hashset.Add(random.Next(1, 999));

            queries[i] = hashset.ToArray();
        }
    }


    void OriginalApproach()
    {
        Compress(true);
    ...
    void BruteForce()
    {
        ...
        segs.Sort();
        var mask = new ulong[n + 2];

        for (int i = 0; i < segs.Count; i += 64)
        {
            if (i > 0) Array.Clear(mask, 0, mask.Length);
            for (int j = Math.Min(i + 63, segs.Count - 1); j >= i; j--)
            {
                var d = j - i;
                var seg = segs[j];
                if (seg.L < mask.Length)
                    mask[seg.L] ^= 1UL << d;
                if (seg.R + 1 < mask.Length)
                    mask[seg.R + 1] ^= 1UL << d;
            }
    ...
    void SolveId2()
    {
        var queryPoints = new List<QueryPoint>[n];
        for (int i = 0; i < queryPoints.Length; i++)
            queryPoints[i] = new List<QueryPoint>();

        for (int iq = 0; iq < queryPoints.Length; iq++)
        {
            var line = queries[iq];
            for (var ip = 0; ip < line.Length; ip++)
            {
                var p = line[ip];
                queryPoints[p].Add(new QueryPoint { QueryIndex = iq, Point = p, PointIndex = ip });
            }
        }

        for (int i = 0; i < n; i++)
        {
            foreach (var qp in queryPoints[i])
            {
    ...
    void Compress(bool reduceSegments = false)
    {
        Rehash(false);
