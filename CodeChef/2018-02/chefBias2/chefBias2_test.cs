...
#pragma warning disable 169
    ...
    public static bool CustomCheck(TextReader input, TextReader output)
    {
      /*  int t = int.Parse(input.ReadLine());
        ...
        while (t-- > 0)
        {
            int[] array = Array.ConvertAll(input.ReadLine().Split(), int.Parse);
            n = array[0];
            m = array[1];
            bounds = new int[m][];

            for (int i = 0; i < m; i++)
                bounds[i] = Array.ConvertAll(input.ReadLine().Split(), int.Parse);

            a = new double[n][];
            for (int i = 0; i < n; i++)
                a[i] = Array.ConvertAll(input.ReadLine().Split(), x => int.Parse(x) / 1000000.0);


            array = Array.ConvertAll(output.ReadLine().Split(), int.Parse);
            for (int i = 0; i < array.Length; i++)
            {
                if (array[i] > bounds[i][1] || array[i] < bounds[i][0])
                    return false;
            }
