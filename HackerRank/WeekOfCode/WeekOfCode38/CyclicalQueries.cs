...
partial class Solution
{
    ...
    void Solve()
    {
        ...
        W = new long[N + 1];
        heap = new SimpleHeap<Pair>[N + 1];
        pairs = new Pair[N + M + 1];

        int pos = 0;
        long sum = 0;
        pairs[pos++] = new Pair(0, 0, 0);
        for (int i = 0; i < N; i++, pos++)
        {
            W[pos] = sum;
            var pair = pairs[pos] = new Pair(sum, i + 1, i + 1);
            heap[pos] = new SimpleHeap<Pair>();
            heap[pos].Enqueue(pair);
            sum += ws[i];
        }

        st = new MaxSegmentTree(pairs);
        ...
        for (int q = 0; q < M; q++)
        {
            ...
            if (c != 2)
            {
                // y = farthest node from x
                var maxLeft = st.GetMaxInclusive(x, N);
                var maxRight = x == 1 ? null : st.GetMaxInclusive(1, x - 1);

                if (maxRight == null
                    || maxLeft.Dist > maxRight.Dist + sum
                    || maxLeft.Dist == maxRight.Dist + sum && maxLeft.Node > maxRight.Node)
                {
                    maxNode = maxLeft;
                }
                else
            ...
            int root = maxNode.Root;
            int far = maxNode.Node;
            long w = (c < 3) ? Nl() : 0;
            long distx = maxNode.Dist + distadd - pairs[x].Dist;

            switch (c)
            {
                case 1:
                case 2:
                    // Add w from y to new node
                    int z = pos++;
                    Add(z, w + maxNode.Dist, root);
                    break;
                case 3:
                    // Delete y
                    Delete(maxNode);
    ...
    void Delete(Pair pair)
    {
        ...
        var set = heap[pair.Root];
        set.Dequeue();
        st[pair.Root] = set.FindMax();
    }

    void Add(int z, long w, int root)
    {
        var pair = pairs[z] = new Pair(w, z, root);
        var set = heap[root];
        set.Enqueue(pair);
        st[root] = set.FindMax();
    }
    ...
    public class Pair : IComparable<Pair>
    {
        ...
        public Pair(long dist, int node, int root)
        {
            Dist = dist;
            Node = node;
            Root = root;
        ...
        {
            int cmp = Dist.CompareTo(pair.Dist);
            return cmp != 0 ? cmp : Node.CompareTo(pair.Node);
        }

        public override string ToString()
        {
            return $"N={Node} D={Dist} R={Root}";
        }
    }
    ...
    public class MaxSegmentTree
    {
        ...
        #region Constructor
        public MaxSegmentTree(Pair[] array)
        {
            int size = array.Length;
            _tree = new Pair[size * 2];
            ...
            {
                _tree[i] = Max(_tree[i << 1], _tree[i << 1 | 1]);
            }
        }

        public Pair Max(Pair x, Pair y)
        {
            if (x == null) return y;
            if (y == null) return x;
        ...
        public Pair this[int index]
        {
            ...
            set
            {
                ...
                int i0 = index + (_tree.Length >> 1);

                _tree[i0] = value;
                for (int i = i0; i > 1;)
                {
                    var v = Max(_tree[i], _tree[i ^ 1]);
                    i >>= 1;
                    if (v.CompareTo(_tree[i]) == 0) break;
                    _tree[i] = v;
                }
            }
        }
        ...
        public Pair GetMaxInclusive(int left, int right)
        {
            ...
            {
                if ((left & 1) == 1) max = Max(max, _tree[left++]);
                if ((right & 1) == 0) max = Max(max, _tree[right--]);
            }
            return max;
        }
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < _tree.Length >> 1; i++)
            {
                sb.Append(this[i]);
                sb.Append(',');
            }
...
public class Program
{
    ...
    static extern int system(string path);

	public static void Main()
	{
		// Recompile using optimizations and `C# 7',
		system("mcs -d:C7 -debug -unsafe -o+ s*.?? >&2; mono --debug *e"); 
		// and exit 'lickety split 
	}
}
#endif // `C7'
