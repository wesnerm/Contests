...
partial class Solution
{
	...
	void Solve()
    {
        ...
        while (left <= right)
        {
            ...
            for (int u = 0; u < n; u++)
            {
                ...
                {
                    mf.AddEdge(u, v, cap);
                    mf.AddEdge(u, v, int.MaxValue, 1);
                }
            }

            var totalCost = mf.GetMaxFlow(0, n - 1, trucks);

            if (totalCost > repairs)
                left = mid + 1;
            else
                right = mid - 1;
        }
    ...
    static void RunOptimized(Action action)
    {
#if C7
        action();
        File.WriteAllText(Environment.GetCommandLineArgs()[1], "");
        Thread.Sleep(300);
#else
        var cs7 = "C7";
        var id = Process.GetCurrentProcess().Id.ToString();
        string comp = $"mcs -d:C7 -debug -o+ -unsafe -r:System.Numerics -out:{cs7} *.cs";
        Process.Start("bash", $"-c \"[ -e {cs7} ] || {comp} >&2; mono --debug {cs7} {id}\"");
        for (int i = 0; i < 40 && !File.Exists(id); i++) Thread.Sleep(99);
#endif
    }
