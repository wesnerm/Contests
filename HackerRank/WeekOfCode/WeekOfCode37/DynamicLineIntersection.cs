...
partial class Solution
{
	...
	void Solve()
	{
        ...
        int[][] locounts = new int[LIGHT][];
        locounts[1] = new int[1];
        for (int ii = 0; ii < n; ii++)
        {
            ...
            if (c == '?')
            {
                ...
                int result = counts[q] + locounts[1][0];
                for (int i = 2; i < LIGHT; i++)
                {
                    var locount = locounts[i];
                    if (locount!=null) 
                        result += locount[q % i];
                }
                WriteLine(result);
            ...
            else
            {
                ...
                if (k >= LIGHT)
                {
                    for (var y = b; y <= MAX; y += k)
                        counts[y] += d;
                    for (var y = b - k; y >= 0; y -= k)
                        counts[y] += d;
                }
                else
                {
                    var locount = locounts[k];
                    if (locount==null) locount= locounts[k]=new int[k];
                    locount[b % k] += d;
                }
            }
        }
