...
partial class Solution
{
    ...
    public void Solve()
    {
        ...
        for (int q = 0; q < Q; q++)
        {
            ...
            else
            {
                int l = Ni()-1;
                int r = Ni()-1;
                var sort = mst.QueryList(l, r);
                var perim = 0L;
                for (int i = 0; i + 3 <= sort.Length; i++)
                {
                    if (sort[i] < 0L + sort[i + 1] + sort[i + 2])
                    {
                        perim = 0L + sort[i] + sort[i + 1] + sort[i + 2];
                        break;
                    }
                }
    ...
    public class MergeSortTree
    {
        ...
        public MergeSortTree(int[] array)
        {
            this.array = array;
            this.n = array.Length;
            tree = new int[4 * n][];
        ...
        int[] QueryList(int node, int start, int end, int i, int j)
        {
            ...
            var answer1 = QueryList(l, start, mid, i, j);
            var answer2 = QueryList(r, mid + 1, end, i, j);
            return Merge(answer1, answer2);
        }

        public int[] QueryList(int i, int j)
        {
            return QueryList(1, 0, n - 1, i, j);
        }
        ...
        public static int[] Merge(int[] array1, int[] array2)
        {
            ...
            size = Math.Min(size, array1.Length + array2.Length);
