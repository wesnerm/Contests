...
partial class Solution
{
    ...
    void Solve()
    {
        ...
        for (int tItr = 0; tItr < t; tItr++)
        {
            ...
            long count2 = Approach2(F.ToArray());
            WriteLine(count2);
        }
    }

    long Approach2(long[] freq)
    {
        long count = 0;
        long leftOver = freq[0];

        for (int i = 1; i < 26; i++)
        {
            var f = freq[i];
            if (f == 0) continue;
            var take = Max(0, Min(leftOver - 1, f));

            var newLeftOver = leftOver - take;
            var newCount = count + take;

            f -= take;

            var prevSize = count * 2;
            var take2 = Max(0, Min(prevSize, f));
            if (take2 > 0)
            {
                newCount -= (take2 + 1) / 2;
                newCount += take2;
    ...
    long Sum(int start, int end, long[] freq)
    {
        long sum = 0;
