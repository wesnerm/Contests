...
class Solution {
	Dictionary<uint, string> dict = new Dictionary<uint, string>(1000000);
	private int k, n, p;
	private long m;
	...
	public void solve()
	{
        ...
        int z = (int)(Math.Ceiling(Math.Log(m,k)))+1;
        int start = Math.Max(0, n-z);
        ...
        while (true)
        {
			...
			if (dict.ContainsKey((uint)cert))
			{
                if (s == dict[(uint)cert]) continue;
                
				WriteLine(prefix + " " + s);
				WriteLine(prefix + " " + dict[(uint)cert]);
				return;
			}
            dict[(uint)cert]=s;
        }
