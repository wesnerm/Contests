...
static partial class CaideTester
{
    ...
    public static bool CustomCheck(TextReader input, TextReader output)
    {
        ...
        for (int i = 0; i < n - 1; i++)
        {
            edges[i] = Array.ConvertAll(input.ReadLine().Split(), int.Parse);
            --edges[i][0];
            --edges[i][1];
        }

        var pts = points.ToArray();
        var good = Solution.Check(pts, edges);

        var score = Solution.Score(pts);
        Solution.Error($"Score: {score:0.00E+0}");
        return good;
    }
