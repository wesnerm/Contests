...
partial class Solution
{
    ...
    public class TreeGraph
    {
        ...
        public int[] NodeSum(Func<int, int> func, int[] reuseBuffer = null)
        {
            var g = Graph;
            var result = reuseBuffer ?? new int[g.Length];
            var queue = Queue;
        ...
        public int[] NodeAgg(Func<int, int> func, Func<int, int, int> combine,
            int[] reuseBuffer = null)
        {
            var g = Graph;
            var result = reuseBuffer ?? new int[g.Length];
            var queue = Queue;
        ...
        public int[] NodeMin(Func<int, int> func, bool minimize = true, int[] reuseBuffer = null)
        {
            var g = Graph;
            var result = reuseBuffer ?? new int[g.Length];
            var queue = Queue;
        ...
        public int[] NodeSet(Func<int, int> func, int[] reuseBuffer = null)
        {
            var g = Graph;
            var result = reuseBuffer ?? new int[g.Length];
            var queue = Queue;
        ...
        public void MinimizeNodeStatistics(int[] childStats, bool minimize = true)
        {
            var g = Graph;
            var parents = Parents;
            var queue = Queue;
        ...
        public struct ExcludedMask
        {
            ...
            public int Exclude(int exclude) => (Mask & ~exclude) | Mask2;
        }


        public void MinimizeParentStatistics(int[] childStats, int[] parentStats,
            bool merge = true)
        {
            int treeSize = TreeSize;
            var g = Graph;
            var queue = Queue;
            var empty = new ExcludedMask();
    ...
    static int Div(int left, int divisor) =>
        left * Inverse(divisor) % MOD;

    static int Add(int x, int y) =>
        (x += y) >= MOD ? x - MOD : x;

    static int Subtract(int x, int y) => (x -= y) < 0 ? x + MOD : x;

    static int Fix(int n) => (n %= MOD) >= 0 ? n : n + MOD;

    static int ModPow(int n, int p, int mod = MOD)
    {
        int b = n;
        int result = 1;
        while (p != 0)
    ...
    static List<int> _fact, _ifact;

    static int Fact(int n)
    {
        if (_fact == null) _fact = new List<int>(FactCache) { 1 };
        for (int i = _fact.Count; i <= n; i++)
            _fact.Add(Mult(_fact[i - 1], i));
        return _fact[n];
    }

    static int InverseFact(int n)
    {
        if (_ifact == null) _ifact = new List<int>(FactCache) { 1 };
    ...
    static void Write(int signedNumber)
    {
