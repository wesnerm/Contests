...
partial class Solution
{
    ...
    public void Solve()
    {
        ...
        for (t = 0; t < testCases.Count; t++)
        {
            ...
            var bucket1 = GradientDescent(RegularGradient);

            int trys = m < 200 ? 6 : 5;
            for (int i = 0; i < trys; i++)
            {
                var b = GradientDescent(RegularGradient);
                if (bucket1.InversionCount > b.InversionCount)
                    bucket1 = b;
            }
            var result1 = bucket1.Assignment;
    ...
    Bucket GradientDescent(bool regular = true)
    {
    ...
    public class Bucket : IComparable<Bucket>
    {
        ...
        int ComputeInversions()
        {
            switch (Approx)
            {
                case 1: return sol.ApproximateInversions(Scores);
                case 2: return sol.ApproximateInversions2(Scores);
            }
    ...
    int ApproximateInversions(long[] arr)
    {
        return ApproximateInversions(arr, buffer1, 0, arr.Length);
    }

    int ApproximateInversions(long[] arr, long[] buffer, int start, int length)
    {
        int end = Min(arr.Length, start + length);
        ...
        var factor = 256.0 / range;
        int count = 0;

        var altBuffer = buffer == buffer1 ? buffer2 : buffer1;
        ft2.Clear();
        Array.Clear(offsets, 0, offsets.Length);

        for (int i = 0; i < arr.Length; i++)
        {
            var b = (int)((arr[i] - min) * factor);
            offsets[b]++;
        }
        ...
        {
            var v = arr[i];
            var b = (int)((v - min) * factor);
            altBuffer[offsets[b]++] = v;
            count += ft2.SumInclusive(b - 1);
            ft2.Add(b, 1);
        }
        ...
        for (int i = 0; i < 256; i++)
        {
            var binend = offsets[i];
            if (binend - binstart <= 2)
            {
                if (binend - binstart == 2 && buffer[binstart] >= buffer[binstart + 1]) count++;
                continue;
            }
            count += ApproximateInversions(buffer, altBuffer, binstart, binend - binstart);
            binstart = binend;
        }
        return count;
    }
    ...
    int ApproximateInversions2(long[] arr)
    {
        int count = 0;
    ...
    int[] ScoresToResult(long[] scores)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            result[i] = (int)Round((1 - alpha) * bounds[i][1] + alpha * bounds[i][0]);
        }
    ...
    void InitTestCases(bool limit = true)
    {
        ...
        if (limit && testCases.Count == 5)
        {
            ...
            testCases[3].TimeLimit = testCases[4].TimeLimit - 2500; // 200
            testCases[2].TimeLimit = testCases[3].TimeLimit - 1000; // 50
