...
partial class Solution
{
        ...
        for (int len = 1; len <= n; len++)
        {
            if (len > 1)
            {
                for (int i = n - 1; i >= len - 1; i--)
                    for (int j = 0; j < m; j++)
                    {
                        mins[i,j] = Min(mins[i, j], mins[i - 1, j]);
                        if (len <= k)
                            mins[i, j] = Min(mins[i, j], sums[i + 1][j] - sums[i + 1 - len][j]);
                    }
            }


            for (int i0 = 0; i0 <= n - len; i0++)
            {
                int i = i0 + len - 1;
                ...
                for (int j = 0; j < m; j++)
                {
                    var v = sums[i + 1][j] - sums[i0][j];

                    // Max Sums Without Zero
                    if (sum <= 0)
                    {
                        if (sum < 0) min = BIG;
                        sum = 0;
                    ...
                    sum += v;
                    min = Min(min, mins[i, j]);

                    if (sum > max && (j0 != 0 || j + 1 < m))
                        max = sum;

                    // Max Sums with Zero
                    sum2 += v;
                    
                    // Convert Max Sum without Zero to Max Sum with Zero
                    if (sum - min > sum2)
                        sum2 = sum - min;
    ...
    void Solve()
    {
        ...
        long result1 = CutAStrip();
        Transpose();
        long result2 = CutAStrip();
    ...
    public static void Write(long signedNumber)
    {
        ...
        do
        {
            outputBuffer[outputIndex++] = (byte)(48 + number % 10);
            number /= 10;
        }
        while (number > 0);
